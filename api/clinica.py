...
def buscarCitas():
    global cursor, conn
    try:
        _DNI = request.get_json('_DNI')
        DNI = (_DNI["_DNI"])
        print(DNI)
        conn = mysql.connect()
        cursor = conn.cursor()
        cursor.callproc('verCitas', (DNI,)) #OBTENER TODAS LAS CITAS PENDIENTES DE UN DETERMINADO PACIENTE
        citas = cursor.fetchall()
        if len(citas) > 0:
            return jsonify(citas)
        else:
            return jsonify(mensaje='No tiene ninguna cita pendiente')
